...
class MediaServiceImp<T extends Media> implements MediaService<T> {
  ...
  saveMediaCollection(collection: Readonly<MediaCollection<T>>): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      this._store
        ...
        .then((value) => {
          console.log(
            `Saved the ${collection.identifier} collection successfully! Saved value: ${value}`
          );
          resolve();
        })
        .catch((err) => {
  ...
  getMediaCollectionIdentifierList(): Promise<string[]> {
    return new Promise<string[]>((resolve, reject) => {
      this._store
        ...
        .catch((err) => {
          console.error(
            `Failed to retrieve the list of media collection identifiers. Error: ${err}`
          );
          reject(err);
        });
    });
  ...
  removeMediaCollection(identifier: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      console.log(
        `Remove media collection with the following identifier ${identifier}`
      );
...
interface MediaManView {
  ...
  clearNewBookCollectionForm(colllectionIdentifier: string): void;
}

class HTMLMedianView implements MediaManView {
  ...
  constructor() {
    ...
    if (!this._newBookCollectionForm) {
      throw new Error(
        "Could not initilaize the view. The 'newBookCollection' element id was not found. was the template changed?"
      );
    }

    if (!this._newBookCollectionName) {
      throw new Error(
        "Could not initilaize the view. The 'newBookCollectionName' element id was not found. was the template changed?"
      );
    }

    if (!this._bookCollectionsContainer) {
      throw new Error(
        "Could not initilaize the view. The 'bookCollections' element id was not found. was the template changed?"
      );
    }
    ...
    <div class="containerGroup">
        <div class="container">
            ...
            <form id="newBook-${bookCollection.identifier}" action="#">
                <ul>
                    <li>
                        <input id="newBookName-${bookCollection.identifier}" type="text"
  ...
  getNewBookDetails(
    collectionIdentifier: string
  ): { error?: string | undefined; book?: Readonly<Book> | undefined } {
    ...
    const newBookNumberOfPages = Number.parseInt(newBookAuthorField.value);
  ...
  removeBook(collectionIdentifier: string, bookIdentifier: string): void {
    ...
    if (!bookElement) {
      throw new Error(
        "The element corresponding to the book to remove could not be found! Did the the template change?"
      );
    }
...
class MediaManControllerImpl implements MediaManController {
  ...
  createBookCollection(): void {
    ...
    this._bookService
      ...
      .then(() => {
        console.log(
          `New book collection called "${newBookCollection.identifier}" saved successfully. Identifier:`,
  ...
  reloadBookCollection(): void {
    this._bookService.getMediaCollectionIdentifierList().then((keys) => {
      ...
      this._view.clearBookCollection();
      keys.forEach((key) => {
  ...
  removeBookCollection(identifier: string): void {
    ...
    this._bookService
      ...
      .then(() => {
        console.log("Remove the collection with identifier: ", identifier);
      })
      .catch((_) => {
  ...
  createBook(collectionIdentifier: string): void {
    ...
    if (
      ...
      console.error(
        "Tried to add a book to an unknowm collection. Identifier: ",
        collectionIdentifier
      );
    ...
    this._bookService
      ...
      .then(() => {
        ...
        this._view.renderBook(existingCollection.identifier, newBook);
      })
      .catch((error) => {
        console.error(
        ...
        this._view.displayErrorMessage(
          `Failed to upda the existing book collection call ${existingCollection.name}`
        );
      });
  }
  ...
  removeBook(collectionIdentifier: string, bookIdentifier: string): void {
    ...
    this._bookService
      ...
      .catch((error) => {
        ...
        this._view.displayErrorMessage(
          `Failed to save the modification mad to the ${existingCollection.name} book collection (removal of the following book: ${bookIdentifier})`
        );
      });
  }
}

const view: HTMLMedianView = new HTMLMedianView();
const bookservice = new MediaServiceImp<Book>(Book);
const movieservice = new MediaServiceImp<Movie>(Movie);
...
console.log("MedianMan ready!", customWindow.mediaManController);
...
// displayGameProperty(game, "players");
